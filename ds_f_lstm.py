...
class DS():
    ...
    # FGSM算法攻击代码
    def get_mask(self, batch_num):
        tensor1d = torch.tensor([1,0,0,0,1])

    # 扩展为十维张量
        tensor10d = tensor1d.unsqueeze(0)

        # 沿着第 0 维重复 30 次
        tensor_repeat = tensor10d.repeat(30, 1)

        # 重塑为二维张量
        tensor2d = tensor_repeat.view(30, 5)
        # 在第 0 维度上扩展一个新的维度
        tensor3d = tensor2d.unsqueeze(0)

        # 沿着第 0 维度重复张量 batch_num 次
        tensor_batch = tensor3d.repeat(batch_num, 1, 1)
    ...
    def fgsm_attack(self, image, epsilon, data_grad):
        ...
        tensor_batch = self.get_mask(image.shape[0])
        x_perturb = epsilon * sign_data_grad
        hadamard_product = torch.mul(x_perturb, tensor_batch)

        perturbed_image = image + hadamard_product
...
def dp():
    ...
    parser.add_argument("--m", type=str, default="lstm", help="select model")
    parser.add_argument("--f", type=str, default="mult_seq", help="select feature")
    parser.add_argument("--h", type=int, default=144, help="first hidden_num")
    parser.add_argument("--b", type=int, default=128, help="batch size")
    parser.add_argument("--e", type=int, default=10, help="epoch num")
    ...
    dir = "./model_save_f_lstm/{}_{}_{}_{}/{}".format(args.d_1, args.d_2, args.f, args.m, system.time_now)
